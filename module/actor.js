...
export class HKActor extends Actor {
  ...
  prepareDerivedData() {
    ...
    // Derived data
    this.system.movement.footwork.value = Math.ceil(Math.floor(Number(this.system.attributes.grace.value)) / 2);

    // Handling modificators (later will add more)
    let cuteTraitMod = 0;
    let spookTraitMod = 0;
    let hungerTraitMod = 0;
    // let totalBulk = 0;
    // let charm_attuned = 0;
    for (let i of this.items) {
      if (i.type === 'trait') {
        cuteTraitMod += Number(i.system.cuteMod);
        spookTraitMod += Number(i.system.spookMod);
        hungerTraitMod += Number(i.system.hunger);
      }
    }
    this.system.appearence.cute.modifier = cuteTraitMod;
    this.system.appearence.spook.modifier = spookTraitMod;
  }
  ...
  getRollData() {
    ...
    //console.log("data after _applyFormulaReplacements: ", data)

    console.log("rollData result: ", data)
    return data;
  }
